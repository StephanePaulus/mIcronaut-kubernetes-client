 ...
 * @since 1.0.0
 */
@Requires(property = CompanyKubernetesConfiguration.PREFIX + ".watch", notEquals = StringUtils.FALSE, defaultValue = StringUtils.TRUE)
@Parallel
@Singleton
public class MountedVolumeWatchThread implements LifeCycle<MountedVolumeWatchThread> {
    ...
    protected MountedVolumeWatchThread(
            ...
            CompanyKubernetesConfiguration configuration) throws IOException {
                                ...
                                for (WatchEvent<?> watchEvent : watchEvents) {
                                    ...
                                    if (kind == StandardWatchEventKinds.OVERFLOW) {
                                        ...
                                        if (context instanceof Path) {
                                            ...
                                            LOG.debug("File at path {} received event: {}.", context, kind);
                                            if(kind.equals(ENTRY_CREATE)
                                                    && context.equals(Path.of(configuration.getContextPath()))) {
    ...
    private void addMountedVolumeWatchDirectory(Path p, boolean isConfigmap) throws IOException {
        WatchKey key = p.register(watchService, ENTRY_CREATE);
        if (isConfigmap) {
